...
class BRReplicatedKVStoreTestAdapter: BRRemoteKVStoreAdaptor {
    ...
    init(testCase: XCTestCase) {
        ...
        db["hello"] = (1, Date(), [0, 1], false)
        db["removed"] = (2, Date(), [0, 2], true)
        for i in 1...20 {
            db["testkey-\(i)"] = (1, Date(), [0, UInt8(i + 2)], false)
        }
    }
    
    func keys(_ completionFunc: ([(String, UInt64, Date, BRRemoteKVStoreError?)], BRRemoteKVStoreError?) -> ()) {
    ...
    func ver(_ key: String, completionFunc: (UInt64, Date, BRRemoteKVStoreError?) -> ()) {
    ...
    func get(_ key: String, version: UInt64, completionFunc: (UInt64, Date, [UInt8], BRRemoteKVStoreError?) -> ()) {
    ...
    func put(_ key: String, value: [UInt8], version: UInt64, completionFunc: (UInt64, Date, BRRemoteKVStoreError?) -> ()) {
    ...
    func del(_ key: String, version: UInt64, completionFunc: (UInt64, Date, BRRemoteKVStoreError?) -> ()) {
...
class BRReplicatedKVStoreTest: XCTestCase {
    ...
    func testBasicSyncGetAllObjects() {
        let exp = expectation(withDescription: "sync")
    ...
    func testSyncTenTimes() {
        let exp = expectation(withDescription: "sync")
        var n = 10
        var handler: (e: ErrorProtocol?) -> () = { e in return }
        handler = { (e: ErrorProtocol?) in
            XCTAssertNil(e)
            if n > 0 {
    ...
    func testSyncPreventsAnotherConcurrentSync() {
        let exp1 = expectation(withDescription: "sync")
        let exp2 = expectation(withDescription: "sync2")
    ...
    func testLocalDeleteReplicates() {
        let exp1 = expectation(withDescription: "sync1")
        store.syncImmediately = false
    ...
    func testRemoteDeleteReplicates() {
        ...
        let exp3 = expectation(withDescription: "sync3")
        store.syncAllKeys { (e) in
            XCTAssertNil(e)
            exp3.fulfill()
        }
